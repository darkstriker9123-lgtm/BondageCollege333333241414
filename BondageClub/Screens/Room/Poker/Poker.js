		...
		if (P.Data == null) {
			...
			if (P.Image != null) {
				...
				if (W >= 800) {
					Y2 = (440 - H) * 0.5;
					Large = true;
					DrawImageEx(P.Image, MainCanvas, (PokerShowPlayer ? 1250 - W / 2 : 1000 - W / 2), Y + Y2 + 50, {Zoom: 1});
				}
				else {
					if (H < 440) Y2 = (440 - H) * 1.25;
					DrawImageEx(P.Image, MainCanvas, X + 215 - W / 2, Y + Y2, {Zoom: 1.25});
				}
			}
...
function PokerRun() {
	...
	if ((PokerTableCards.length > 0) && ((PokerMode == "FLOP") || (PokerMode == "TURN") || (PokerMode == "RIVER") || (PokerMode == "RESULT"))) {
		for (let C = 0; C < PokerTableCards.length; C++)
			DrawImageEx(PokerCardFileName(PokerTableCards[C]), MainCanvas, C * 150 + 640, 850, {Zoom: 0.6});
	}
...
function PokerKeyDown(event) {
	...
	if ((event.key === "B" || event.key === "b") && modeCheck && handSize > 0) {
		...
		return true;
	} else if ((event.key === "R" || event.key === "r") && modeCheck && handSize > 0) {
		PokerProcess("Raise"); // R to raise
		return true;
	} else if ((event.key === "F" || event.key === "f") && modeCheck && handSize > 0) {
		PokerProcess("Fold"); // F to fold
		return true;
	} else if ((event.key === "W" || event.key === "w") && modeCheck && handSize <= 0) {
		PokerProcess("Watch"); // W to watch
		return true;
	} else if ((event.key === "D" || event.key === "d") && PokerMode === "RESULT") {
