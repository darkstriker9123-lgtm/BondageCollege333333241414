 ...
 */
function LARPCanPlayClubCard() { return (!Player.IsRestrained() && !CurrentCharacter.IsRestrained() && !Player.IsGagged() && !CurrentCharacter.IsGagged()); }

/**
 * Loads the LARP introduction room NPC
 * @type {ScreenLoadHandler}
 */
async function LARPLoad() {
...
function LARPClick() {
	...
	if (MouseIn(1885, 265, 90, 90) && (ReputationGet("LARP") >= 1) && Player.Game?.LARP?.Class && Player.CanChangeOwnClothes()) {
		ChatSearchStart(ChatRoomSpaceType.MIXED, ["Room", "LARP"], { Background: "WrestlingRing", Game: "LARP" });
	}
