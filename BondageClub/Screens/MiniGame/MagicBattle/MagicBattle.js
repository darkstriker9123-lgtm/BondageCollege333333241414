 ...
 */
async function MagicBattleLoad() {
}
...
function MagicBattleGetDifficulty(C) {
	let D = 0;
	let NF = ((C.IsPlayer()) && (ReputationGet("HouseAmplector") >= 100) && LogQuery("Mastery", "MagicSchool")) ? 0 : 1; // House Amplector masters aren't affected by nudity
	let BF = ((C.IsPlayer()) && (ReputationGet("HouseMaiestas") >= 100) && LogQuery("Mastery", "MagicSchool")) ? 0.5 : 1; // House Maiestas masters are half affected by bondage items
	...
	if (InventoryIsWorn(C, "ItemArms", "HempRope")) D = D + 1 * BF;
	if (InventoryIsWorn(C, "ItemArms", "Chains")) D = D + 3 * BF;
	if (InventoryIsWorn(C, "ItemArms", ["LeatherArmbinder", "ArmbinderJacket", "BoxTieArmbinder", "LatexArmbinder", "SeamlessLatexArmbinder"])) D = D + 4 * BF;
	if (InventoryIsWorn(C, "ItemTorso", "HempRopeHarness")) D = D + 2 * BF;
	if (InventoryIsWorn(C, "ItemTorso", "CrotchChain")) D = D + 4 * BF;
	return D;
}
...
function MagicSpellEffect(C, Spell) {
	...
	if (Spell == 2) {
		if (InventoryGet(C, "ItemArms") == null) {
			...
			const item = InventoryWear(C, "HempRopeHarness", "ItemTorso");
			TypedItemSetOptionByName(C, item, "Diamond", true);
		}
	}
 ...
 */
function MagicBattleKeyDown(_event) {
	return false;
}
