...
function KinkyDungeonLock(item, lock) {
	const group = /** @type {AssetGroupItemName} */(KDRestraint(item).AssetGroup ? KDRestraint(item).AssetGroup : KDRestraint(item).Group);
	if (lock != "") {
		if (KinkyDungeonIsLockable(KDRestraint(item))) {
			item.lock = lock;
			if (lock == "Gold") item.lockTimer = MiniGameKinkyDungeonLevel + 2;
			if (!StandalonePatched)
				InventoryLock(KinkyDungeonPlayer, InventoryGet(KinkyDungeonPlayer, group), "IntricatePadlock", "A kinky lock", true);
			item.pickProgress = 0;
			if (ArcadeDeviousChallenge && InventoryGet(KinkyDungeonPlayer, group) && !KinkyDungeonRestraintsLocked.includes(group)) {
				InventoryLock(Player, InventoryGet(Player, group), "IntricatePadlock", "A deviously kinky lock", false);
				KinkyDungeonPlayerNeedsRefresh = true;
			}
		}
	} else {
		item.lock = lock;
		InventoryUnlock(KinkyDungeonPlayer, group);
...
function KinkyDungeonStruggle(struggleGroup, StruggleType, index) {
	...
	if ((StruggleType == "Struggle") && !data.hasAffinity && data.escapeChance <= 0 && data.escapeChance >= -edgeBonus && (!KDRestraint(restraint).alwaysEscapable || !KDRestraint(restraint).alwaysEscapable.includes(StruggleType))) {
		let typesuff = "";
		if (KDToggles.Sound) AudioPlayInstantSoundKD(KinkyDungeonRootDirectory + "Audio/Struggle.ogg");
...
function KDCanAddRestraint(restraint, Bypass, Lock, NoStack, r, Deep, noOverpower, securityEnemy, useAugmentedPower) {
	...
	if (restraint.Group == "ItemNipplesPiercings" && !KinkyDungeonStatsChoice.get("arousalModePiercing")) return false;
	...
	if (restraint) {
		if (!KDGroupBlocked(restraint.Group, true) || Bypass || KDEnemyPassesSecurity(restraint.Group, securityEnemy)) {
			...
			if (!KinkyDungeonCancelFlag) {
				...
				const lockedItem = InventoryGet(Player, AssetGroup);
				const lockItem = InventoryGetLock(lockedItem);
				if (ArcadeDeviousChallenge && KinkyDungeonDeviousDungeonAvailable() && !KinkyDungeonRestraintsLocked.includes(AssetGroup)
					&& AssetGroup !== "ItemHead" && InventoryAllow(Player, AssetGet("Female3DCG", AssetGroup, restraint.Asset)) &&
					(!lockItem || !(lockItem.Asset.OwnerOnly || lockItem.Asset.LoverOnly))) {
