			...
			if (StandalonePatched) {
				...
				if (save) {
					KDGetDressList().Default.push({
						...
						Property: C.Appearance[A].Property,
						Color: /** @type {ItemColor} */ ((C.Appearance[A].Color) ? C.Appearance[A].Color : (C.Appearance[A].Asset.DefaultColor ? C.Appearance[A].Asset.DefaultColor : "Default")),
						Lost: false,
					},);
				}
			}
...
function KinkyDungeonDressPlayer(Character, NoRestraints) {
	...
	try {
		...
		if (KDGameData.KneelTurns > 0 || KDGameData.SleepTurns > 0) {
			if (StandalonePatched) {
				...
				if (!PoseSetByItems(Character, "BodyLower", "Kneel") && !Character.IsKneeling()) {
					PoseSetActive(Character, "Kneel", false);
				}
			}
			...
			if (StandalonePatched) {
				...
				if (!PoseSetByItems(Character, "BodyLower", "Kneel") && Character.IsKneeling()) {
					PoseSetActive(Character, "BaseLower", false);
				}
			}
		...
		if (!StandalonePatched) {
			...
			let Eyes2 = undefined;
			/** @type {ExpressionName} */
			let Eyebrows = undefined;
			/** @type {ExpressionName} */
			let Mouth = undefined;
			/** @type {ExpressionName} */
			let Fluids = undefined;
			...
			if (KDToggles.Drool && KDGameData.KinkyDungeonLeashedPlayer > 0) {
				if (Fluids?.includes("Drool")) Fluids = /** @type {ExpressionName} */ (Fluids.replace("Drool", "DroolTears"));
	...
	if (StandalonePatched && KDCurrentModels.get(Character)) {
		...
		if (!KDInventoryGet(Character, "Body")) KDInventoryWear("Body", "Body");
		if (!KDInventoryGet(Character, "Eyes")) KDInventoryWear("KoiEyes", "Eyes");
		if (!KDInventoryGet(Character, "Brows")) KDInventoryWear("KoiBrows", "Brows");
		if (!KDInventoryGet(Character, "Mouth")) KDInventoryWear("KoiMouth", "Mouth");
		if (!KDInventoryGet(Character, "Blush")) KDInventoryWear("KoiBlush", "Blush");
...
/**
 * @param {Character} C
 * @param {KDAssetGroupName} group
 * @returns
 */
function KDInventoryGet(C, group) {
	return InventoryGet(C, /** @type {AssetGroupName} */ (group));
}
...
function KDInventoryWear(AssetName, AssetGroup, par, color, filters) {
	...
	const A = AssetGet(KinkyDungeonPlayer.AssetFamily, /** @type {AssetGroupName} */ (AssetGroup), AssetName);
	...
	let item = StandalonePatched ?
		...
		: KDAddAppearance(KinkyDungeonPlayer, AssetGroup, A, /** @type {ItemColor} */ (color || A.DefaultColor));
...
function KDApplyItemLegacy(inv, tags) {
	...
	try {
		...
		if (!restraint.armor || KDToggles.DrawArmor) {
			placed = KDAddAppearance(KinkyDungeonPlayer, AssetGroup, AssetGet("Female3DCG", AssetGroup, restraint.Asset), color, undefined, undefined, undefined, inv);
		}
		...
		if (placed) {
			...
			// Convert old-style `Type` into `TypeRecord`
			if (restraint.Modules) {
				const data = ExtendedItemGetData(placed.Asset, ExtendedArchetype.MODULAR);
				if (data) {
					typeRecord = Object.fromEntries(data.modules.map((mod, i) => [mod.Key, restraint.Modules[i]]));
				}
			} else if (restraint.changeRenderType && Object.keys(restraint.changeRenderType).some((k) => {return tags.has(k);})) {
				let key = Object.keys(restraint.changeRenderType).filter((k) => {return tags.has(k);})[0];
				if (key) {
					typeRecord = ExtendedItemTypeToRecord(placed.Asset, restraint.changeRenderType[key]);
				}
			} else if (restraint.Type) {
				typeRecord = ExtendedItemTypeToRecord(placed.Asset, restraint.Type);
			}

			ExtendedItemInit(KinkyDungeonPlayer, placed, false, false);
			if (Object.keys(typeRecord).length) {
				ExtendedItemSetOptionByRecord(KinkyDungeonPlayer, placed, typeRecord, { push: false, refresh: false });
			}

			placed.Property ??= {};
			placed.Property.LockedBy = inv.lock ? "MetalPadlock" : undefined;
			placed.Property.Difficulty = restraint.power;
			if (restraint.OverridePriority) {
				placed.Property.OverridePriority = restraint.OverridePriority;
			}
			CharacterRefresh(KinkyDungeonPlayer, false, false);
		}
	} finally {
		// @ts-expect-error
