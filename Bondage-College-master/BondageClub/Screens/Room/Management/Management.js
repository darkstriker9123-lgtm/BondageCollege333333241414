 ...
 */
function ManagementOwnerFromBondageCollege() { return Player.IsOwned() === "npc" && ["Sidney", "Amanda", "Jennifer"].includes(Player.OwnerName()); }
/**
 ...
 */
function ManagementOwnerAway() { return !(Player.IsOwned() === "npc" && ["Sidney", "Amanda", "Jennifer"].includes(Player.OwnerName())); }
/**
 ...
 */
function ManagementCanBreakTrialOnline() { return Player.IsOwned() === "online" && !Player.IsFullyOwned(); }
/**
 ...
 */
function ManagementCanBeReleasedOnline() { return Player.IsOwned() === "online" && Player.OwnedSince() >= 7 && Player.GetDifficulty() <= 2; }
/**
 ...
 */
function ManagementCannotBeReleasedOnline() { return Player.IsOwned() === "online" && Player.OwnedSince() < 7 && Player.GetDifficulty() <= 2; }
/**
 ...
 */
function ManagementCanBeReleased() { return Player.IsOwned() === "npc" && !PrivateOwnerInRoom(); }
/**
 ...
 */
function ManagementCannotBeReleased() { return Player.IsOwned() === "npc" && PrivateOwnerInRoom() && Player.GetDifficulty() <= 2; }
/**
 ...
 */
function ManagementCannotBeReleasedExtreme() { return Player.IsOwned() === "npc" && PrivateOwnerInRoom() && Player.GetDifficulty() >= 3; }
/**
 ...
 */
function ManagementWillOwnPlayer() { return (!Player.IsOwned() && (ReputationGet("Dominant") <= -100) && (ManagementMistressAngryCount == 0) && PrivateHasEmptySlot() && ManagementNoMistressInPrivateRoom()); }
/**
 ...
 */
function ManagementWontOwnPlayer() { return (!Player.IsOwned() && (ReputationGet("Dominant") <= -1) && (ReputationGet("Dominant") >= -99) && PrivateHasEmptySlot() && ManagementNoMistressInPrivateRoom()); }
/**
 ...
 */
function ManagementIsClubSlave() { return (InventoryIsWorn(Player, "ItemNeck", "ClubSlaveCollar")); }
/**
 ...
 */
function ManagementCanTransferToRoom() { return PrivateHasEmptySlot(); }
/**
 ...
 */
function ManagementCompleteContract() { return (!LogQuery("ClubSlave", "Management") && !Player.IsKneeling()); }
/**
 ...
 */
function ManagementKneelingCompleteContract() { return (!LogQuery("ClubSlave", "Management") && Player.IsKneeling()); }
/**
...
function ManagementNoMistressInPrivateRoom() {
	for (const char of PrivateCharacter) {
		if (char.IsNpc() && char.Title === "Mistress") {
			return false;
		}
	}
	return true;
...
async function ManagementLoad() {
	...
	if ((ManagementMistress == null) && (TextGet("Mistress") != "")) {
		...
		ManagementSub = CharacterLoadNPC("NPC_Management_Sub", null, "Room", "Management");
		...
		PoseSetActive(ManagementSub, "Kneel", true);
...
function ManagementClick() {
	...
	if (MouseIn(1885, 265, 90, 90) && Player.CanKneel()) PoseSetActive(Player, Player.ActivePoseMapping.BodyLower !== "Kneel" ? "Kneel" : "BaseLower", true);
}
...
function ManagementContactOwner() {
	...
	if (Player.IsOwned() === "npc" && Player.OwnerName() === "Sidney") ManagementCanReleaseChastity = (Math.random() >= 0.6);
}
...
function ManagementReleaseFromOwner(RepChange) {
	if (!(Player.IsOwned() === "npc" || Player.IsOwned() === "online")) return;
...
function ManagementBreakLover(L) {
	Player.Lover = "";
	if (Player.Lovership[L].MemberNumber) {
		ServerSend("AccountLovership", { MemberNumber: Player.Lovership[L].MemberNumber, Action: "Break" });
	} else {
		ServerSend("AccountLovership", { MemberNumber: -1, Name: Player.Lovership[L].Name, Action: "Break" });
	}
	ServerPlayerSync();
}
...
function ManagementFindClubSlaveRandomIntro() {
	...
	var Intro = (Math.floor(Math.random() * 7) + 1).toString();
	...
	if (Intro == "7") {
		ManagementRandomGirlArchetype = "Bunny";
		CharacterArchetypeClothes(ManagementRandomGirl, "Bunny");
		ShibariRandomBondage(ManagementRandomGirl, Math.floor((Math.random() * 4) + 1));
	}
	if (Intro != "6") {
